...
function ShowJobListingMenu(data)
	ESX.TriggerServerCallback('esx_lesterjobs:getJobsList', function(data)
		local elements = {}
		for i = 1, #data, 1 do
			table.insert(
				elements,
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				TriggerServerEvent('esx_lesterjobs:setJob', data.current.value)
				TriggerEvent("esx_lesterjobs:notify", "CHAR_LESTER", 1, "ESX Lester Jobs", _U('Lester_Jobs'), _U('new_job'))
				menu.close()
			end,
			function(data, menu)
				menu.close()
...
AddEventHandler('esx_lesterjobs:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
end)

RegisterNetEvent("esx_lesterjobs:notify")
AddEventHandler("esx_lesterjobs:notify", function(icon, type, sender, title, text)
    Citizen.CreateThread(function()
		Wait(1)
...
Citizen.CreateThread(function()
	while true do
		...
		if not isInJoblistingMarker and hasAlreadyEnteredMarker then
			...
			TriggerEvent('esx_lesterjobs:hasExitedMarker')
		end
	end
end)
